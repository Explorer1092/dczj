...
public class StyleService {
	 ...
	 * @return
	 */
	public String annexInfoQues(String strContent) {
		String tmqs = "<!--unitname=附件区-->";
		String tmqe = "<!--/unitname=附件区-->";
		int begin = strContent.indexOf(tmqs)+19;
		int end = strContent.indexOf(tmqe);
		strContent = strContent.substring(begin, end);
		return strContent;
	...
	public String annexInputQues(String strContent){
		String tmqs = "<!--unitname=附件题-->";
		String tmqe = "<!--/unitname=附件题-->";
		int begin = strContent.indexOf(tmqs)+19;
		int end = strContent.indexOf(tmqe);
		strContent = strContent.substring(begin, end);
		return strContent;
...
//						}
				...
				if(quesType == 10){
					...
					String content = "";
					styleCode = this.getStyleCodeByPath(quespage);
					content = this.annexInfoQues(styleCode);
					content = content.replaceAll("<!--quesname-->", quesname)
							         .replaceAll("<!--divquesId-->", quesId+"");
					strInner += content;  //题目类型为小节时。获取题目内容
					continue;
...
//							option = "<input id='surveyAnsw_" + quesId + "_" + iid + "' name='surveyAnsw" + quesId + "_content' class='" + textCss + "'/>";
	...
	public String parseQues(String content, int quesId,int type) {
		...
		if(m1.find()){  
			...
			for(AnswInfo en : answList){
				...
				if (type==8) {
					...
					text = text.replaceAll("<!--defaultvalue-->",defaultvalue)
