...
public class OprQuesController {
	...
	public JsonResult submitAddQuesAndAnsw(int dczjid, int type) {
		...
		if(newquesid > 0) {
			...
			if(type == 0 || type == 1||type==6||type==7||type==8) {
				...
				String[][] filetypeStrings = 
					{
						{"限制上传文件大小：","4","0"},
						{"限制上传文件类型：","1"},
						{"图片文件：","2"},{".gif","7"},{".png","7"},{".jpg","7"},{".jpeg","7"},
						{"文档文件：","3"},{".doc","8"},{".docx","8"},{".pdf","8"},{".xls","8"},{".xlsx","8"},{".ppt","8"},{".pptx","8"},{".txt","8"},
						{"压缩文件：","4"},{".rar","9"},{".zip","9"},
						{"视频文件：","5"},{".mp4","10"},{".avi","10"},{".mov","10"},
						{"音频文件：","6"},{".mp3","11"},{".amr","11"},{".waw","11"}
					};
				for (int i = 0; i < filetypeStrings.length; i++) {
					...
					answEn.setAllowfillinair(0);
					answEn.setAnswnote("");
					answEn.setAnswname(filetypeStrings[0][i]);
					answEn.setDefaultvalue(0);
					answEn.setType(NumberUtil.getInt(filetypeStrings[0][i]));
					int answid = answInfoService.add(answEn);
					if(answid > 0) {
						jsonResult.addParam("answid" + i, answid);
					}
	...
	public JsonResult edit(Integer dczjid,Integer quesid) {
		...
		if(type == 0){   //单选题
			...
			middleContent = styleService.parseQues(content,quesid,0);
			...
			beforeContent = content.substring(0,beforeIndex);
			afterContent = content.substring(afterIndex);
			middleContent = styleService.parseQues(content,quesid,1);
			...
			beforeContent = content.substring(0,beforeIndex);
			afterContent = content.substring(afterIndex);
			middleContent = styleService.parseQues(content,quesid,8);
			...
			content = styleService.annexInputQues(content);
			String strContent = "";
			String beforeContent = "";
			String middleContent = "";
			String afterContent = "";
			...
			beforeContent = content.substring(0,beforeIndex);
			afterContent = content.substring(afterIndex);
			middleContent = styleService.parseQues(content,quesid,10);
			...
			beforeContent = content.substring(0,beforeIndex);
			afterContent = content.substring(afterIndex);
			middleContent = styleService.parseQues(content,quesid,6);
			...
			beforeContent = content.substring(0,beforeIndex);
			afterContent = content.substring(afterIndex);
			middleContent = styleService.parseQues(content,quesid,7);
